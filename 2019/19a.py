...
path = os.path.join(os.getcwd(), "data", "input_19a.txt")
...
for x in range(700, 1000):
    for y in range(0, 1000):
        relative_base = 0
        print(x, y)
        output, state = run_intcodes(puzzle_input.copy(), [x, y], relative_base, debug=False)
        outputs.append(output)
        outputs_str.append(str(output))
    outputs_str.append('\n')

print(f"Total affected squares: {sum(outputs)}")

# outputs_str = [str(o) for o in outputs]

path = os.path.join(os.getcwd(), "data", "output_19a_3.txt")
with open(path, "w") as f:
    f.writelines(outputs_str)