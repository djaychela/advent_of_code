...
# create grid
grid = [["." for _ in range(min_x - max_y, max_x + 1 + max_y + 5)] for _ in range(-1 , max_y + 1)]
# add floor
grid.append(['#' for _ in range(min_x - max_y, max_x + 1 + max_y + 5)])
...
for rock_line in rock_lines:
    for idx in range(0, len(rock_line) - 1):
        ...
        for coord in coords:
            grid[coord[1]][coord[0] - min_x + 1 + max_y] = "#"
...
def add_grain(grid):
    ...
    while falling:
        # check below
        try:
            if grid[c_loc[1]+1][c_loc[0] - min_x + 1 + max_y] == ".":
                c_loc[1] += 1
            # check below and left
            elif grid[c_loc[1]+1][c_loc[0] - min_x + max_y] == ".":
                c_loc[1] += 1
                c_loc[0] -= 1
            # check below and right
            elif grid[c_loc[1]+1][c_loc[0] - min_x + 2 + max_y] == ".":
                c_loc[1] += 1
                c_loc[0] += 1
            else:
                if c_loc == [500, 0]:
...
while True:
    ...
    if grain:
        grid[grain[1]][grain[0] - min_x + 1 + max_y] = "O"
        total_grains += 1
    else:
        # leaving grid
        break

# account for 'blocked nozzle':
total_grains += 1 
